...
//		}
	...
	private void addKey() {	
		...
		int y = 100;
		fill(180, 38, 8);
		triangle(x, y, x-0.5f*CityMarker.TRI_SIZE, y+CityMarker.TRI_SIZE, x+0.5f*CityMarker.TRI_SIZE, y+CityMarker.TRI_SIZE);
		
		fill(255);
		ellipse(x, 130, CityMarker.TRI_SIZE, CityMarker.TRI_SIZE);
		rectMode(CENTER);
		fill(255);
		rect(x, 155, CityMarker.TRI_SIZE, CityMarker.TRI_SIZE);
		
		fill(255, 255,0);
		ellipse(x, 210, CityMarker.TRI_SIZE, CityMarker.TRI_SIZE);
		fill(0, 0, 255);
		ellipse(x, 235, CityMarker.TRI_SIZE, CityMarker.TRI_SIZE);
		fill(255, 0, 0);
		ellipse(x, 260, CityMarker.TRI_SIZE, CityMarker.TRI_SIZE);
		
		line(x-0.5f*CityMarker.TRI_SIZE, 285-0.5f*CityMarker.TRI_SIZE, x+0.5f*CityMarker.TRI_SIZE, 285+0.5f*CityMarker.TRI_SIZE);
		line(x+0.5f*CityMarker.TRI_SIZE, 285-0.5f*CityMarker.TRI_SIZE, x-0.5f*CityMarker.TRI_SIZE, 285+0.5f*CityMarker.TRI_SIZE);
	...
	private void printQuakes() 
	{
		...
		// use a map to optimize the efficient later, dropped by Benson.
		
		// count the quake for each country
		for (Marker cm : countryMarkers) {
			int quakeCount = 0;
			...
			for (Marker qm : quakeMarkers) {
				...
				if (em.isOnLand()) {
					...
					String countryQuake = lm.getCountry();
					if (country == countryQuake) {
						quakeCount++;
					}	
				}
			}
			if (quakeCount > 0) {
				System.out.println("country: " + country + ", the number of earthquakes: " + quakeCount);
			}	
		}
		
		// count the quake in ocean
		int quakeCount = 0;
		for (Marker qm : quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker)qm;
			if (!em.isOnLand()) {
		...
		System.out.println("In ocean, the number of earthquakes: " + quakeCount);
